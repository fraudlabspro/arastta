...
class ModelAntifraudFraudLabsPro extends Model {
    public function check($data) {
        ...
        if (!$this->config->get('fraudlabspro_antifraud_status') ||!$this->config->get('fraudlabspro_antifraud_key')) {
            return;
        }
        ...
        if (filter_var($this->config->get('fraudlabspro_antifraud_simulate_ip'), FILTER_VALIDATE_IP)) {
            $ip = $this->config->get('fraudlabspro_antifraud_simulate_ip');
        }

        $request['key'] = $this->config->get('fraudlabspro_antifraud_key');
        $request['ip'] = $ip;
        ...
        $request['format'] = 'json';
        $request['source'] = 'arastta';
        $request['source_version'] = '1.1.1';
        ...
        if (is_null($json = json_decode($response)) === FALSE) {
            $this->db->query("REPLACE INTO `" . DB_PREFIX . "order_fraudlabspro` SET order_id = '" . (int)$data['order_id'] . "',
                ...
                api_key = '" .  $this->config->get('fraudlabspro_antifraud_key') . "',
                ip_address = '" .  $ip . "'"
            );
        ...
        if ($risk_score > $this->config->get('fraudlabspro_antifraud_score')) {
            return $this->config->get('fraudlabspro_antifraud_order_status_id');
        }

        if ($json->fraudlabspro_status == 'REVIEW') {
            return $this->config->get('fraudlabspro_antifraud_review_status_id');
        }

        if ($json->fraudlabspro_status == 'APPROVE') {
            return $this->config->get('fraudlabspro_antifraud_approve_status_id');
        }

        if ($json->fraudlabspro_status == 'REJECT') {
            return $this->config->get('fraudlabspro_antifraud_reject_status_id');
        }
    }
    
    public function getFraud($order_id) {
        $query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order_fraudlabspro` WHERE order_id = '" . (int)$order_id . "'");
